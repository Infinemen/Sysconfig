...
# 更新状态栏
refresh() {
    _icons=''; _wifi=''; _cpu=''; _mem=''; _date=''; _vol=''; _bat=''; _wea='';   # 重置所有模块的状态为空
    source $tempfile                                                              # 从 temp 文件中读取模块的状态
    xsetroot -name "$_icons$_wifi$_cpu$_mem$_date$_vol$_bat$_wea"                 # 更新状态栏
}
...
cron() {
    ...
    while true; do
        to=()                                                                       # 存放本次需要更新的模块
        [ $((i % 300)) -eq 0 ]   && to=(${to[@]} wifi)                              # 每 10秒   更新 wifi
        [ $((i % 30)) -eq 0 ]    && to=(${to[@]} cpu mem vol icons)                 # 每 20秒   更新 cpu mem vol icons
        [ $((i % 3600)) -eq 0]   && to=(${to[@]} wea)                               # 每 3600秒 更新 weather
        [ $((i % 5)) -eq 0 ]     && to=(${to[@]} date)                              # 每 5秒    更新 date
        [ $((i % 300)) -eq 0 ]   && to=(${to[@]} bat)                               # 每 300秒  更新 date
        [ $i -lt 30 ]            && to=(wifi cpu mem date vol icons bat wea)        # 前 30秒   更新所有模块
...
case $1 in
    ...
    updateall|check) update icons wifi cpu mem date vol bat wea ;;
